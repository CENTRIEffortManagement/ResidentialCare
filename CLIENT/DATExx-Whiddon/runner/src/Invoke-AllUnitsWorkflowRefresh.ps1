...
try {
    ...
    if (-not (Test-Path -LiteralPath $excelRefreshScript -PathType Leaf)) {
        $script:ExitCode = 10
        throw "Required refresh engine is missing: runner/src/Invoke-AllUnitsExcelWorkbookRefresh.ps1"
    }

    $resolvedRunnerRoot = (Resolve-Path -LiteralPath (Join-Path $PSScriptRoot "..") -ErrorAction Stop).Path
    if (-not (Test-IsUnderRoot -Path $resolvedSequencePath -Root $resolvedRunnerRoot)) {
        $script:ExitCode = 10
        throw "Sequence path must live under the Date-level runner root: $resolvedSequencePath"
    }
