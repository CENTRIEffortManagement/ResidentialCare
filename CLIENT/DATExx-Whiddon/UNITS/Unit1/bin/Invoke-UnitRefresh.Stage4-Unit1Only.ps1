...
$sequencePath = Join-Path $runnerRoot "Unit-WorkbookSequence.psd1"
...
function Show-Status {
    ...
    if (Test-Path -LiteralPath $statusPath -PathType Leaf) {
        ...
        if ($workflowLine.Count -gt 0 -and $workflowLine[0] -ne "workflow=ResidentialCare Unit Refresh") {
            Write-Host "No current ResidentialCare Unit Refresh status file exists yet."
        }
        else {
    ...
    if (Test-Path -LiteralPath $runLogsPath -PathType Container) {
        $latestLog = Get-ChildItem -LiteralPath $runLogsPath -Filter "UnitRefresh-*.log" -File -ErrorAction SilentlyContinue |
        ...
        if ($latestLog) {
            ...
            Write-Host "No ResidentialCare Unit Refresh log exists yet."
        }
...
function Write-StopRequest {
    ...
    $lines = @(
        ...
        "source=ResidentialCare Unit Refresh menu"
    )
...
function Show-Menu {
    while ($true) {
        Write-Host ""
        Write-Host "ResidentialCare Unit Refresh"
        Write-Host "============================"
...
try {
    ...
    Assert-PackageFile -Path $sequencePath -Description "unit workbook sequence file"
