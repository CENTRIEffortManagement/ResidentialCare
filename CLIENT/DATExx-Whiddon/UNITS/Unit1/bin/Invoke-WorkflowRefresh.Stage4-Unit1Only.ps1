...
$script:WorkflowName = "ResidentialCare Unit Refresh"
$script:LogFile = $null
...
function Acquire-RunLock {
    ...
    try {
        ...
        $script:ExitCode = 13
        throw "Another ResidentialCare Unit Refresh appears to be running. Lock file is in use: $LockPath"
    }
}
...
function Resolve-WorkbookEntries {
    param(
        [Parameter(Mandatory = $true)]
    ...
    if ($entries.Count -eq 0) {
        $script:ExitCode = 11
        throw "Sequence file is invalid: Workbooks is empty."
    }
    ...
    foreach ($entry in $entries) {
        ...
        if ($sequence -lt 1) {
            $script:ExitCode = 11
            throw "Sequence file is invalid: sequence numbers must be 1 or greater."
        }
        ...
        if ([string]::IsNullOrWhiteSpace($folder) -or [System.IO.Path]::IsPathRooted($folder) -or $folder -match '(^|[\\/])\.\.([\\/]|$)') {
            $script:ExitCode = 11
            throw "Sequence file is invalid: folder must be a safe Unit1-relative path for $fileName."
        }

        if ($allowedFolders -notcontains $folder) {
            $script:ExitCode = 11
            throw "Sequence file is invalid: folder '$folder' is not allowed for ResidentialCare Unit Refresh workbook $fileName."
        }
        ...
        if ($allowedRelativePaths -notcontains $relativePath) {
            $script:ExitCode = 11
            throw "Sequence file is invalid: workbook '$relativePath' is not in the approved ResidentialCare Unit Refresh sequence."
        }
        ...
        $workbookPath = Join-Path (Join-Path $ResolvedRunRoot $folder) $fileName
        if (-not (Test-IsUnderRoot -Path $workbookPath -Root $ResolvedRunRoot)) {
            $script:ExitCode = 11
            throw "Sequence file is invalid: workbook path escapes run root for $fileName."
        }
        ...
        if ($folder -match '[\\/]') {
            ...
            $qualifiedName = "$roleName / $fileName"
        }
...
function Resolve-WorkbookStart {
    ...
    if ($exactMatches.Count -gt 1) {
        $script:ExitCode = 11
        throw "Start workbook '$WorkbookInput' is ambiguous. Matches: $(Join-WorkbookDisplayNames -Entries $exactMatches). Use a role-qualified workbook name such as 'AIN / CapacityDistrib(A.1)-shifts.xlsx' or use -StartAtSequence."
    }
    ...
    if ($partialMatches.Count -gt 1) {
        $script:ExitCode = 11
        throw "Start workbook '$WorkbookInput' is ambiguous. Matches: $(Join-WorkbookDisplayNames -Entries $partialMatches). Use a role-qualified workbook name such as 'AIN / CapacityDistrib(A.1)-shifts.xlsx' or use -StartAtSequence."
    }

    $script:ExitCode = 11
    throw "Start workbook '$WorkbookInput' was not found in the configured workbook sequence."
}
    ...
    if ($StartAtSequence -gt 0) {
        ...
        if ($selected.Count -eq 0) {
            ...
            if ($EndAtSequence -gt 0) {
                throw "Sequence range $StartAtSequence-$EndAtSequence does not match any configured workbook."
            }

            throw "StartAtSequence $StartAtSequence is after the end of the workbook sequence."
        }

        if ($selected[0].Sequence -ne $StartAtSequence) {
            $script:ExitCode = 11
            throw "Start sequence $StartAtSequence was not found in the configured workbook sequence."
        }

        if ($EndAtSequence -gt 0 -and $selected[-1].Sequence -ne $EndAtSequence) {
            $script:ExitCode = 11
            throw "End sequence $EndAtSequence was not found in the configured workbook sequence."
        }
...
function Assert-SelectedWorkbookFiles {
    ...
    foreach ($entry in $SelectedEntries) {
        if (-not (Test-Path -LiteralPath $entry.Path -PathType Leaf)) {
            $script:ExitCode = 12
            throw "Selected workbook is missing: $($entry.QualifiedName) ($($entry.RelativePath))"
        }

        $entry.Path = (Resolve-Path -LiteralPath $entry.Path -ErrorAction Stop).Path

        if (Test-ExcelTempLockExists -Path $entry.Path) {
            $script:ExitCode = 12
            throw "Selected workbook appears open in Excel because a temporary lock file exists: $($entry.QualifiedName) ($($entry.RelativePath))"
        }

        if (-not (Test-WorkbookWritable -Path $entry.Path)) {
            $script:ExitCode = 12
            throw "Selected workbook is not writable for refresh: $($entry.QualifiedName) ($($entry.RelativePath))"
        }
    }
}
...
try {
    ...
    Assert-RequiredFolder -Root $resolvedRunRoot -RelativePath "1. Input" | Out-Null
    ...
    $entries = Resolve-WorkbookEntries -SequenceData $sequenceData -ResolvedRunRoot $resolvedRunRoot
    ...
    if ($ValidateSelectionOnly) {
        ...
        foreach ($entry in $selectedEntries) {
            Write-Host ("[{0}] {1} ({2})" -f $entry.Sequence, $entry.QualifiedName, $entry.RelativePath)
        }
        exit 0
    }
    ...
    $runLockPath = Join-Path $runLogsPath "UnitRefresh.lock"
    Acquire-RunLock -LockPath $runLockPath

    $oldRefreshLogs = @(Get-ChildItem -LiteralPath $runLogsPath -Filter "UnitRefresh-*.log" -File -ErrorAction SilentlyContinue)
    foreach ($oldRefreshLog in $oldRefreshLogs) {
        Remove-Item -LiteralPath $oldRefreshLog.FullName -Force
    }

    $timestamp = Get-Date -Format "yyyyMMdd-HHmmss"
    $script:LogFile = Join-Path $runLogsPath ("UnitRefresh-{0}.log" -f $timestamp)
    ...
    Write-Log "Starting ResidentialCare Unit Refresh workflow refresh package."
    Write-Log "Run root: $resolvedRunRoot"
    ...
    foreach ($entry in $selectedEntries) {
        Write-Log ("[{0}] {1} ({2})" -f $entry.Sequence, $entry.QualifiedName, $entry.RelativePath)
    }
    ...
    foreach ($entry in $selectedEntries) {
        ...
        Write-Log ("Refreshing sequence {0}: {1}" -f $entry.Sequence, $entry.QualifiedName)
        ...
        $arguments = @(
            ...
            "-StatusPrefix", ("[{0}/{1}] {2}:" -f $entry.Sequence, $entries.Count, $entry.QualifiedName),
        ...
        if ($childExitCode -ne 0) {
            ...
            throw "Workbook refresh failed for $($entry.QualifiedName). Exit code: $childExitCode"
        }

        Write-Log ("Workbook refresh succeeded: {0}" -f $entry.QualifiedName)
    ...
    Write-Log "ResidentialCare Unit Refresh workflow refresh succeeded."
