...
$sequencePath = Join-Path $runnerRoot "Role-WorkbookSequence.psd1"
...
function Show-Sequence {
    try {
        ...
        foreach ($entry in @($data.Workbooks | Sort-Object Sequence)) {
            $roleName = Get-RoleName -Folder ([string] $entry.Folder)
            Write-Host ("{0,2}. {1} / {2}" -f $entry.Sequence, $roleName, $entry.FileName)
        }
    }
    catch {
...
function Show-Status {
    ...
    if (Test-Path -LiteralPath $statusPath -PathType Leaf) {
        ...
        if ($workflowLine.Count -gt 0 -and $workflowLine[0] -ne "workflow=ResidentialCare Roles") {
            Write-Host "No current ResidentialCare Roles status file exists yet."
        }
        else {
    ...
    if (Test-Path -LiteralPath $runLogsPath -PathType Container) {
        $latestLog = Get-ChildItem -LiteralPath $runLogsPath -Filter "RoleRefresh-*.log" -File -ErrorAction SilentlyContinue |
        ...
        if ($latestLog) {
            ...
            Write-Host "No ResidentialCare Roles refresh log exists yet."
        }
...
function Write-StopRequest {
    ...
    $lines = @(
        ...
        "source=ResidentialCare Roles menu"
    )
...
function Show-Menu {
    while ($true) {
        Write-Host ""
        Write-Host "ResidentialCare Roles Refresh"
        Write-Host "============================="
        ...
        switch ($choice) {
            ...
            "2" {
                ...
                Write-Host ""
                Show-Sequence
                $workbook = Read-Host "Enter role-qualified workbook name, partial name, or file name"
...
try {
    ...
    Assert-PackageFile -Path $sequencePath -Description "role workbook sequence file"
