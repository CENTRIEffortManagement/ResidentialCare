...
// Pathname: c:\Users\Alex\CentriNOTSYNC\ResidentialCare\CLIENT\DATExx-Whiddon\UNITS\TE\1. Input\Demand-MasterRoster Manual Read.xlsx
// Extracted: 2026-09-21T06:31:13.532Z
...
// Query: Unit
// Purpose: Use the resolved workbook folder name as the unit filter value.
shared Unit =
let
...
// Query: IMPORT Master
// Purpose: Read the current unit's Master Roster source through the resolved unit path.
shared #"IMPORT Master" = let
    Source = Excel.Workbook(File.Contents(Unit1Path & "\1. Input\Allocation\Master Roster.xlsx"), null, true),
...
// Purpose: Read the Whiddon SharePoint facility reference used by unit-level roster filtering.
...
// Purpose: Map each source Location title to its canonical Facility-Abbrev.
...
// Purpose: Read the existing Whiddon facility-analysis controls from their SharePoint list.
...
// Query: Master Unit Rows
// Purpose: Retain only Master Roster rows whose linked facility matches this workbook's unit folder.
// Notes: Source Location is retained for the existing historical facility calculations; NR is not excluded by name.
shared #"Master Unit Rows" =
let
    Source = #"IMPORT Master",
    JoinedFacilities = Table.NestedJoin(Source, {"Location"}, #"LINK Facilities", {"Title"}, "FacilityLookup", JoinKind.LeftOuter),
    ...
    Result = Table.RemoveColumns(FilteredUnit, {"Facility-Abbrev"})
in
    Result;
...
// Inputs: Master Unit Rows and MW Roster Role Mapping Prepare.
...
let
    Source = Table.SelectColumns(#"Master Unit Rows", {"Location", "Role"}),
...
// Inputs: Master Unit Rows, MW Roster Role Mapping Prepare, ShftEnd Prepare, and shift-boundary scalars.
...
// Inputs: INPUT TargetMinutes and the source locations retained by Master Unit Rows.
...
let
    ...
    TargetPeriodDays = 14,
    MinutesPerHour = 60,
    // Targets are stored by source facility code. Match the same locations retained
    // from the List-filtered roster; other facilities in the wide input are out of scope.
    SourceFacilityCodes = List.Buffer(List.Distinct(List.RemoveNulls(List.Transform(
        #"Master Unit Rows"[Location],
        each if _ = null then null else Text.Upper(Text.Start(Text.Trim(_), 2)))))),
    AvailableFacilityColumns = List.RemoveItems(Table.ColumnNames(Source), {"MinuteType"}),
    MissingFacilityColumns = List.Difference(SourceFacilityCodes, AvailableFacilityColumns),
    FacilityColumns = if List.IsEmpty(SourceFacilityCodes) then
            error "The Facilities List filter retained no Master Roster locations for this unit."
        else if not List.IsEmpty(MissingFacilityColumns) then
            error Error.Record("Missing unit targets", "TargetMinutes is missing source facility columns retained by the unit filter.", MissingFacilityColumns)
        else SourceFacilityCodes,
...
// Inputs: INPUT TargetMinutes, the unit-scoped MW TargetMinutes Prepare, and MW DayShift Allocation.
...
let
    ...
    // Reconcile only the target columns selected for this unit; unrelated raw
    // facility columns have no corresponding allocation after the unit filter.
    FacilityColumns = List.Buffer(List.Distinct(#"MW TargetMinutes Prepare"[Facility])),
