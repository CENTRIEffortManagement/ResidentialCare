...
// Purpose: Resolve this workbook through the canonical FilePathUrl input and CentriSyncPaths mapping.
// Inputs: FilePathUrl, as either a one-row FilePath table or a single named cell exposed as Column1.
// Output: Variable Name / Value rows describing the resolved workbook folder.
...
let
    ...
    let
        ...
        PathInput =
            if Table.HasColumns(Source, {"FilePath"}) then
                Table.SelectColumns(Source, {"FilePath"})
            else if Table.ColumnNames(Source) = {"Column1"} then
                Table.RenameColumns(Source, {{"Column1", "FilePath"}})
            else
                error "FilePathUrl must be a one-row FilePath table or a single named cell.",
        ValidatedRows =
            if Table.RowCount(PathInput) = 1 then
                PathInput
            else
    ...
    RawFilePath =
        if not Value.Is(RawFilePathValue, type text) then
