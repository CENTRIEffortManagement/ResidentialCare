...
// Pathname: c:\Users\Alex\CentriNOTSYNC\ResidentialCare\CLIENT\DATExx-Whiddon\UNITS\TE\2. Calculations\Capacity-ShiftAvailability.xlsx
// Extracted: 2026-09-24T22:55:30.168Z
...
shared #"IMPORT AllocationExtracted" = let
    Navigation = Excel.Workbook(File.Contents(#"FilePath - 1Input" & "\1-AllocationExtracted.xlsx"), null, true),
    Matches = Table.SelectRows(Navigation, each [Item] = "AllocationExtracted" and [Kind] = "Table"),
    Allocation = if Table.RowCount(Matches) = 1 then Matches{0}[Data]
        else error Error.Record("Allocation import", "Expected exactly one AllocationExtracted table.", [Matches = Table.RowCount(Matches)]),
    RequiredColumns = {"Location", "Code", "Role", "Hours", "Name"},
    MissingColumns = List.Difference(RequiredColumns, Table.ColumnNames(Allocation)),
...
shared ReconciledWorkers_Prepare = let
    ...
    MappedRole = Table.AddColumn(RenamedRole, "Role", each
        ...
        else if [SourceRole] = "Enrolled Nurse" then "EN" else null, type nullable text),
    ...
    Eligibility = Table.AddColumn(NamedWorkers, "Issue", each
        ...
        else if [#"Facility-Abbrev"] <> Unit or not AnalysisEnabled then "Outside unit analysis scope"
...
shared AllocationRoleHours_Prepare = let
    ApprovedRoles = Table.SelectRows(#"IMPORT AllocationExtracted", each List.Contains({"AIN", "EN", "RN"}, [Role])),
    ...
    ParsedHours = Table.AddColumn(UnitRows, "AllocatedHours", each
        try Number.From([Hours]) otherwise null, type nullable number),
    InvalidRows = Table.SelectRows(ParsedHours, each [Code] = null or [Name] = null
        or [AllocatedHours] = null or (if [AllocatedHours] = null then false
            else [AllocatedHours] < 0 or Number.IsNaN([AllocatedHours]) or Number.Abs([AllocatedHours]) = #infinity)),
    ValidatedRows = if Table.IsEmpty(InvalidRows) then ParsedHours
            ...
            Table.SelectColumns(InvalidRows, {"Location", "Code", "Role", "Name", "Hours"})),
    GroupedHours = Table.Group(ValidatedRows, {"Code", "Facility-Abbrev", "Role"},
        {{"AllocatedHours", each List.Sum([AllocatedHours]), type number},
         {"RosterNames", each List.Distinct([Name]), type list},
         {"SourceFacilities", each List.Sort(List.Distinct([Location])), type list}}),
    ResolvedNames = Table.AddColumn(GroupedHours, "RosterName", each
        if List.Count([RosterNames]) = 1 then [RosterNames]{0}
        else error Error.Record("Allocation role name", "An employee/facility/role has conflicting roster names.",
            [EmployeeID = [Code], Facility = [#"Facility-Abbrev"], Role = [Role],
             SourceFacilities = [SourceFacilities], Names = [RosterNames]]), type text),
...
shared AllocationRoleShares = let
    ...
    ValidatedTotals = if Table.IsEmpty(Table.SelectRows(ExpandedTotals, each [TotalHours] <= 0)) then ExpandedTotals
...
shared ReconciledWorkers_Eligible = let
    ...
    AllocationRoleCounts = Table.Distinct(Table.SelectColumns(AllocationRoleShares,
    ...
    DisplayNames = Table.AddColumn(JoinedAllocationRoles, "Name", each
        if [Roles]{0}[RoleCount] > 1 or
            (not Table.IsEmpty([AllocationRoles]) and [AllocationRoles]{0}[RoleCount] > 1)
            then [BaseName] & " (" & [Role] & ")"
...
in Table.Buffer(Table.RemoveColumns(PreferredRole, {"Roles", "AllocationRoles"}));

// Query: WorkerIdentityFailures_DIAGNOSTICS
// Purpose: Identify the worker rows behind the four blocking reconciliation checks.
...
shared WorkerIdentityFailures_DIAGNOSTICS = let
    ...
    PreferredRoleCounts = Table.Group(Workers, {"EmployeeID", "Facility-Abbrev"},
        {{"Rows", each Table.RowCount(_), Int64.Type},
         {"Roles", each Text.Combine(List.Sort(List.Distinct([Role])), ", "), type text}}),
    ...
    Combined = Table.Combine({WorkerKeyDetails, PreferredRoleDetails, PublishedNameDetails, ContractDetails}),
    Result = Table.SelectColumns(Combined,
        {"Check", "EmployeeID", "Facility-Abbrev", "Role", "Name", "Details"})
in Table.Sort(Result, {{"Check", Order.Ascending}, {"EmployeeID", Order.Ascending}, {"Role", Order.Ascending}});
...
shared WorkerShiftRoleCoverage_DIAGNOSTICS = let
    ShiftCounts = Table.Group(AvailabilityShiftWindows, {"Role"},
        {{"ShiftWindowCount", each Table.RowCount(_), Int64.Type}}),
    ...
    CountedShifts = Table.AddColumn(MatchedRoles, "ShiftWindowCount", each
        if Table.IsEmpty([ConfiguredShifts]) then 0 else [ConfiguredShifts]{0}[ShiftWindowCount], Int64.Type),
    MissingShiftWindow = Table.AddColumn(CountedShifts, "MissingShiftWindow", each
        [ShiftWindowCount] = 0, type logical),
...
shared WorkerShiftDayAssessment = let
    ...
    DayAssessment = Table.AddColumn(JoinedLeaveDays, "DayAssessment", each if Table.IsEmpty([LeaveDay]) then
    ...
    DayColumns = Table.ExpandRecordColumn(Table.RemoveColumns(DayAssessment, {"LeaveDay"}), "DayAssessment",
...
shared WorkerShiftSegments = let
    Measured = Table.AddColumn(WorkerShiftDayAssessment, "Measurement", each AvailabilityShiftMeasurement(
...
shared ResDayShift_CHECK = let
    ...
    Checks = {
        ...
        AvailabilityCheckResult("Eligible worker roles have roster shifts", () =>
            Table.RowCount(Table.SelectRows(WorkerShiftRoleCoverage_DIAGNOSTICS, each [MissingShiftWindow]))),
        AvailabilityCheckResult("Settings shift endpoints", () =>
        ...
        AvailabilityCheckResult("Published facility and roles", () => Table.RowCount(Table.SelectRows(Calculated,
            each [#"Facility-Abbrev"] <> Unit or not List.Contains({"RN", "AIN", "EN"}, [Role]))))
    },
...
shared ResDayShift = let
    ...
    Checked = if Table.IsEmpty(Failures) then ResDayShift_Calculated
            ...
            "Required availability checks failed: " & Text.Combine(Failures[Check], "; ") & ". Inspect ResDayShift_CHECK.", Failures),
...
shared AllocationRoleShares_CHECK = let
    ...
    Checks = {
        AvailabilityCheckResult("Unique allocation role-share keys", () => Table.RowCount(Shares) -
            Table.RowCount(Table.Distinct(Shares, {"EmployeeID", "Facility-Abbrev", "Role"}))),
        AvailabilityCheckResult("Allocation role shares sum to one", () =>
            Table.RowCount(Table.SelectRows(WorkerTotals, each Number.Abs([ShareSum] - 1) > 0.00000001)))
    }
in Table.FromRecords(Checks, type table [Check = text, Status = text, Failures = nullable number, Details = nullable text]);
...
in Table.Sort(Output, {{"Name", Order.Ascending}, {"MultiRole", Order.Ascending}});
...
shared RoleResDayAvailabilityCapped = let
    ...
    AllocationRoleCounts = Table.Distinct(Table.SelectColumns(ValidatedShares,
        {"EmployeeID", "Facility-Abbrev", "RoleCount"})),
    ...
    MissingOrDuplicateShares = Table.SelectRows(JoinedShares, each
        Table.RowCount([AllocationShare]) > 1 or
        (Table.IsEmpty([AllocationShare]) and not Table.IsEmpty([AllocationRoles])
            and [AllocationRoles]{0}[RoleCount] > 1)),
    ValidatedMatches = if Table.IsEmpty(MissingOrDuplicateShares) then JoinedShares
            ...
            Table.SelectColumns(MissingOrDuplicateShares, {"ID", "Facility-Abbrev", "Role", "Name"})),
    RoleShares = Table.AddColumn(ValidatedMatches, "RoleShare", each
        if Table.IsEmpty([AllocationShare]) then 1 else [AllocationShare]{0}[RoleShare], type number),
    // Preserve the prior cap rule: only counts above the Settings maximum are apportioned by role share.
    Capped = Table.AddColumn(RoleShares, "AvailabilityCapped", each
        if [Count] <= [StdRosterDays] then Number.From([Count])
        else Number.From([StdRosterDays]) * [RoleShare], type number),
...
shared #"Availability-StaffList" = let
    ...
    Workers = if Table.IsEmpty(Failures) then ReconciledWorkers_Eligible
            ...
            "Required worker identity checks failed: " & Text.Combine(Failures[Check], "; ") &
                ". Inspect ReconciledWorkers_CHECK and WorkerIdentityFailures_DIAGNOSTICS.", Failures),
...
shared #"IMPORT CentriSyncPaths" =
    Excel.Workbook(File.Contents("C:\Users\Public\Public Scripts\CentriSyncPaths.xlsx"), null, true);
