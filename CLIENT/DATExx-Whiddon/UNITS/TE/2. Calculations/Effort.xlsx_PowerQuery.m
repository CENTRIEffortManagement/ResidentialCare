...
// Pathname: c:\Users\Alex\CentriNOTSYNC\ResidentialCare\CLIENT\DATExx-Whiddon\UNITS\TE\2. Calculations\Effort.xlsx
// Extracted: 2026-09-24T08:06:25.054Z
...
shared RoleShiftCapacity = let
    Source = #"EXTRACT AvailabilityDeveloped",
    ...
    #"Merged Queries" = Table.NestedJoin(#"Renamed Columns1", {"Period"}, DateShiftPeriod, {"Period"}, "PermutationDimensions", JoinKind.LeftOuter),
    #"Expanded PERMUTATIONS DATE SHIFT" = Table.ExpandTableColumn(#"Merged Queries", "PermutationDimensions", {"Date", "Shift"}, {"Date", "Shift"}),
...
shared #"EffortAllMatrixAG1-1D" = let
    Source = Table.NestedJoin(RoleShiftCapacity, {"Date", "Role", "Shift"}, #"EXTRACT DemandCorrected", {"Date", "Role", "ShiftPeriod"}, "Demand", JoinKind.FullOuter),
...
shared #"EffortDAMatrixAG1-1D" = let
    Source = Table.NestedJoin(RoleShiftAllocationDay, {"Date", "ShiftPeriod", "Role"}, #"EXTRACT DemandCorrected", {"Date", "ShiftPeriod", "Role"}, "RoleDATEShiftDemand", JoinKind.LeftOuter),
...
// Notes: Compatibility reference only; Demand.xlsx is opened once by IMPORT Demand.
shared RoleDATEShiftDemand = #"EXTRACT DemandCorrected";

shared ResShiftAllocation = let
    Source = Table.NestedJoin(#"EXTRACT ResourceShiftAllocation", {"Name", "Role"}, #"EXTRACT Table_Masterlist", {"Name", "Role"}, "Table_Masterlist", JoinKind.LeftOuter),
    #"Expanded Table_Masterlist" = Table.ExpandTableColumn(Source, "Table_Masterlist", {"Name", "Resource"}, {"Name.1", "Resource"}),
    #"Removed Other Columns" = Table.SelectColumns(#"Expanded Table_Masterlist",{"ShiftPeriod", "Role", "TimeDate", "ResShiftFTE", "Resource"}),
...
// Purpose: Preserve the established workbook interface used by existing Excel connections.
// Notes: Compatibility reference only; Capacity.xlsx is opened once by IMPORT Capacity.
shared AvailabilityDeveloped2 = #"EXTRACT AvailabilityDeveloped";
...
shared AINContractResources_Prepare = let
    Filtered = Table.SelectRows(#"EXTRACT Table_Masterlist", each [Source] = "Availability" and [Role] = "AIN" and [PreferredRole] = "AIN"),
...
// Inputs: FilePathUrl, as either a one-row FilePath table or a single named cell exposed as Column1.
...
let
    ...
    PathSource = Excel.CurrentWorkbook(){[Name="FilePathUrl"]}[Content],
    ...
    RawFilePath = if not Value.Is(SinglePath, type text) then
        error "FilePathUrl[FilePath] must contain the saved workbook path as text."
        else NormalizePath(SinglePath),
    NonBlankFilePath = if RawFilePath = "" then
        error "FilePathUrl[FilePath] is blank. Save Effort.xlsx and recalculate its CELL filename formula."
        else RawFilePath,
