...
function Show-Sequence {
    try {
        ...
        foreach ($entry in @($data.Workbooks | Sort-Object Sequence)) {
            Write-Host ("{0,2}. {1}" -f $entry.Sequence, $entry.FileName)
        }
    }
    catch {
...
function Show-Status {
    ...
    if (Test-Path -LiteralPath $runLogsPath -PathType Container) {
        $latestLog = Get-ChildItem -LiteralPath $runLogsPath -Filter "RN-refresh-*.log" -File -ErrorAction SilentlyContinue |
        ...
        if ($latestLog) {
            ...
            Write-Host "No ResidentialCare RN refresh log exists yet."
        }
...
function Write-StopRequest {
    ...
    $lines = @(
        ...
        "source=ResidentialCare RN menu"
    )
...
function Show-Menu {
    while ($true) {
        Write-Host ""
        Write-Host "ResidentialCare RN Refresh"
        Write-Host "=========================="
        ...
        switch ($choice) {
            ...
            "2" {
                ...
                Write-Host ""
                Show-Sequence
                $workbook = Read-Host "Enter workbook name, partial name, or file name"
            ...
            "4" {
                ...
                Write-Host ""
                Show-Sequence
                $rangeText = Read-Host "Enter inclusive sequence range, for example 1-4 or 10-14"
...
try {
    ...
    Assert-PackageFile -Path $sequencePath -Description "RN workbook sequence file"
