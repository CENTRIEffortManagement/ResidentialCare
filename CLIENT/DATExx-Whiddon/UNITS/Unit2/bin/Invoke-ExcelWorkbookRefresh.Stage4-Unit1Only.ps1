...
try {
    ...
    if ($useCleanupGhostExcelProcesses -or $useForceCloseExcelProcesses) {
        throw "ResidentialCare Unit Refresh safe mode does not close or kill pre-existing Excel processes."
    }
    ...
    if (Test-ExcelTempLockExists -Path $resolvedWorkbookPath) {
        throw "Workbook appears open in Excel because a temporary lock file exists: $resolvedWorkbookPath. ResidentialCare Unit Refresh safe mode will not close Excel processes."
    }

    if (-not (Test-WorkbookWritable -Path $resolvedWorkbookPath)) {
        throw "Workbook is not writable for refresh: $resolvedWorkbookPath. ResidentialCare Unit Refresh safe mode will not close Excel processes."
    }
