...
let
    ...
    // This table is an inclusion list. NA and unmatched roster roles remain
    // outside MinuteWorker allocation without being treated as input errors.
...
let
    ...
    // MatchingRosterRoleswithANACCRoles is an inclusion list. Validate the
    // supplied mappings below, but do not require every Master Roster role to
    // appear: absent roles are intentionally outside MinuteWorker analysis.
...
// Purpose: Publishes enabled role-group FTE demand from the validated canonical-facility allocation.
// Inputs: MinuteWorkersFTE_TABLE, EXTRACT Roles and RolesAnalysed.
// Output: Facility, fortnight day, FTE, WeekdayShift and Role Group.
shared MinuteWorkersFTE_Demand = let
    Source = MinuteWorkersFTE_TABLE,
    #"Merged Queries" = Table.FuzzyNestedJoin(Source, {"DC Role"}, #"EXTRACT Roles", {"DC Role"}, "EXTRACT Roles", JoinKind.LeftOuter, [IgnoreCase=true, IgnoreSpace=true]),
    #"Expanded EXTRACT Roles" = Table.ExpandTableColumn(#"Merged Queries", "EXTRACT Roles", {"Role Group"}, {"Role Group"}),
    #"Merged Queries1" = Table.NestedJoin(#"Expanded EXTRACT Roles", {"Role Group"}, RolesAnalysed, {"Roles"}, "RolesAnalysed", JoinKind.LeftOuter),
    #"Expanded RolesAnalysed" = Table.ExpandTableColumn(#"Merged Queries1", "RolesAnalysed", {"Effort Management Analysis"}, {"Effort Management Analysis"}),
    #"Filtered EMA ROLES" = Table.SelectRows(#"Expanded RolesAnalysed", each ([Effort Management Analysis] = true)),
    #"Removed Other Columns" = Table.SelectColumns(#"Filtered EMA ROLES",{"Facility", "DayOfWeek", "Week Day", "FortnightDayIndex", "FortnightDay", "FTE", "WeekdayShift", "Role Group"})
in
