...
| `1. Input/AvailabilitiesExtracted.xlsx` | Retained adapter reading the same availability/leave source. Current Capacity-ShiftAvailability reads the source directly. | Review consumers before changing this adapter; do not assume it is the active capacity input. |
