# ParallelInputs-Unit1Priority runner instructions

This is the separately named copy of the original runner instructions. The
profile-specific instructions here take precedence over the copied reference below.

## Selection and scheduling

Use the existing Date-level batch launcher with
`-SequenceProfile ParallelInputs-Unit1Priority`. Omission keeps Current.
The saved roles are AIN, AINC4 and RN; use the saved profile for the authorized
Unit1 trial and subsequent all-Units run.

A1, S1 and D1 use static inputs as confirmed by the operator. Admit all ready
Unit1 work first, then fill residual slots from Unit2 and further Units in numeric
order. Recheck priority at every vacancy; never preempt running work or reserve
slots for unready work. Organisation jobs follow ready Unit jobs. Seven is the
hard ceiling; files inside a batch remain sequential.

Use actual file-level dependencies, not the earlier diagram's whole-batch
barriers. Intervals releases A2; Staff releases the first organisation assembly.
Only three startup dependency edges differ from Current. The catalogue supports
ReadJobs independently of Depends, plus Inputs and settings AdditionalInputs.
Actual declared producer reads still imply readiness and write exclusion.
The static source inventory has not been independently inspected in workbooks;
existing undeclared imports remain unverified. No workbook or M code is changed.

## Portable invocation and reporting

Paths below are relative to the repository root. The launcher locates the runner
relative to itself; the runner resolves configuration from its project root.
A fresh run is supported after moving the complete project. Resume after changing
the host or root is rejected. Resume automatically selects its recorded profile.
Old engine fingerprints are not force-migrated.

```powershell
./CLIENT/DATExx-Whiddon/Run-BatchRefreshRunner.cmd -SequenceProfile ParallelInputs-Unit1Priority -Units Unit1 -ShowPlan -ExportGantt
./CLIENT/DATExx-Whiddon/Run-BatchRefreshRunner.cmd -SequenceProfile ParallelInputs-Unit1Priority -RunAll -Units Unit1 -ValidateSelectionOnly
./CLIENT/DATExx-Whiddon/Run-BatchRefreshRunner.cmd -SequenceProfile ParallelInputs-Unit1Priority -RunAll -Units Unit1
./CLIENT/DATExx-Whiddon/Run-BatchRefreshRunner.cmd -SequenceProfile ParallelInputs-Unit1Priority -RunAll -ValidateSelectionOnly
./CLIENT/DATExx-Whiddon/Run-BatchRefreshRunner.cmd -SequenceProfile ParallelInputs-Unit1Priority -RunAll
./CLIENT/DATExx-Whiddon/Run-BatchRefreshRunner.cmd -ReportRun RUN-ID
```

The all-Units invocation includes organisation work and refreshes Unit1 again.
Execute it only after Unit1 finishes every selected file, has no uncertain save or
recovery snapshot, and its actual report is available and verified. A trial failure
stops progression. Preview and validation never start Excel.

Each run freezes forecast.json and planned.mmd before worker startup. Actual
attempt records preserve retries; actual.json and actual.mmd are generated after
execution. ReportRun regenerates reports only and cannot refresh workbooks.
Chart generation failure does not change refresh results. Report errors block the
trial's automatic progression until reporting is repaired.
Run-specific copies are published beneath the configured reportFolder.

Forecasts use the latest successful recorded workbook duration, not batch averages.
They use the same admission check as execution, with a virtual clock. At the first
unknown-duration admission the timed forecast stops, because unknown slot
occupancy can alter later starts. Remaining jobs are untimed; the total is unknown.
An actual chart includes only observed intervals. Timestamps represent coordinator
dispatch and receipt acknowledgement, including process overhead. Active interval
time is the union of worker intervals, not their summed durations. Gap time
includes coordinator idle time and pauses between resumes.
Missing attempt endpoints are reported as incomplete, not invented.

Keep docs/batch-duration-gantt.mmd unchanged. Its original 17m 23s forecast used
unverified O3/O4 durations, coarse barriers and exceeded seven concurrent jobs.
Retain it only as an illustrative baseline; comparison is not proof of improvement.
No business reconciliation is inferred from technical refresh success.

## Original instructions (reference copy)
